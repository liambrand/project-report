\part{Analysis}
		...
		\chapter{Developing The Robot}
			...
			\section{Movement}
			Not much is needed to achieve movement. First we'll need the chassis which will be composed of a supporting structure as well as a set of wheels and respective motors. Then we require a battery source, and finally a microcontroller which will control the motors.
			
				\subsection{Chassis}
				With regards to the chassis' structure most anything will do, we simply need something with enough space to and support to carry the power source, the microcontroller and any additional electronics we'll use for the robot's observation. The wheels we use for the chassis will be omnidirectional. Whilst a regular wheel can only roll forwards or backwards, omni directional wheels feature smaller wheels (often referred to as rollers) as part of the wheel itself. This allows the wheel to frictionlessly slide in other directions. 
				
				\begin{figure}
					\centering
					\includegraphics[width=.3\linewidth]{ANALYSIS/90degwheel.png}
					\caption{An omnidirectional wheel}
				...
				To control the direction and speed of the robot's wheels, a microcontroller will be required. It will most likely be linked up to an appropriate power source, and depending on the chassis configuration it may be directly connected to the wheel's motors. From here it will control the power being supplied to the wheels, modifying it in order to change the direction and speed of the wheels depending on what direction/orientation we want to steer the robot in.
