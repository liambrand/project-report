...
This chapter will discuss the testing performed on the system. The purpose of the testing is to determine how well, if at all, elements of the implemented work satisfy the requirements outlined in \ref{requirements}. Each test will have its process and purpose described, and based on the results there will be a discussion of how it affected development. These results will also be used for the product's evaluation. 
	...
	Discussion of tests and their respective results will be broken down into two categories in this section. Unit testing and integration testing. Unit tests are focused on testing specific pieces of code, often functions, with an already established insight on what the piece of code actually needs to do. Verifying that code works as expected ensures that source code is sound and works properly before it is modified or built on to implement new things. Integration testing is much higher level, and relates to testing the created system against outlined requirements. It will involve observing the system's behavior after performing certain actions to it to see if the response is what is expected. This requires no knowledge of the inner workings of the product, and allows for a simple comparison of what the product does verses what it should do.
	
	Full test plans are included as an appendix. Relevant code snippets for any programs created to test functionality are also part of the appendix. What follows is an explanation of the tests carried out and their conclusions.
	
	\section{Integration Testing}
	Integration tests were used to verify the presence of key functionality which was outlined in the product's requirements. Tests will be explain and conclusions based on the test results will be discussed. Relevant code snippets and tables containing more specific tests results can be found in the appendices.
